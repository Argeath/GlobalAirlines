	...
	public function action_index() {
		...
		if ($flights->count() > 0) {
			foreach ($flights as $flight) {
				...
				$flightsText .= '<td><span class="Jtooltip" data-container=".main" data-toggle="tooltip" data-placement="bottom" title="' . Helper_Map::getCityName($flight->from) . '">' . HTML::anchor("airport/index/" . $flight->from, cleanString(Helper_Map::getCityCode($flight->from), false, " ")) . '</a></td>';
				$flightsText .= '<td><span class="Jtooltip" data-container=".main" data-toggle="tooltip" data-placement="bottom" title="' . Helper_Map::getCityName($flight->to) . '">' . HTML::anchor("airport/index/" . $flight->to, cleanString(Helper_Map::getCityCode($flight->to), false, " ")) . '</td>';
				$flightsText .= "<td>" . Helper_TimeFormat::timestampToText($flight->end) . $delayed . "</td>";
				$flightsText .= "<td class='zegarCountdown' czas='" . $flight->end . "' now='" . time() . "'>" . Helper_TimeFormat::secondsToText($flight->end - time()) . "</td>";
				$flightsText .= "</tr>";
			}
		} else {
		...
		if ($departures->count() > 0) {
			foreach ($departures as $departure) {
				...
				$odlotyText .= '<td><span class="Jtooltip" data-container=".main" data-toggle="tooltip" data-placement="bottom" title="' . Helper_Map::getCityName($departure->from) . '">' . HTML::anchor("airport/index/" . $departure->from, cleanString(Helper_Map::getCityCode($departure->from), false, " ")) . '</a></td>';
				$odlotyText .= '<td><span class="Jtooltip" data-container=".main" data-toggle="tooltip" data-placement="bottom" title="' . Helper_Map::getCityName($departure->to) . '">' . HTML::anchor("airport/index/" . $departure->to, cleanString(Helper_Map::getCityCode($departure->to), false, " ")) . '</td>';
				$odlotyText .= "<td>" . Helper_TimeFormat::timestampToText($departure->end) . $delayed . "</td>";
				$odlotyText .= "<td class='zegarCountdown' czas='" . $remainingTime . "' now='" . time() . "'>" . Helper_TimeFormat::secondsToText($remaining) . "</td>";
				$odlotyText .= "</tr>";
			}
		} else {
		...
		if ($other->count() > 0) {
			...
			foreach ($other as $o) {
				...
				$otherText .= "<td class='zegarCountdown' czas='" . $o->when . "' now='" . time() . "'>" . Helper_TimeFormat::secondsToText($o->when - time()) . "</td>";
				$otherText .= "</tr>";
			}
		}
